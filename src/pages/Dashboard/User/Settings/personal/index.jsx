  ...
  return (
    <Container onSubmit={handleSubmit(updateInfo)}>
      <TopFrame>
        ...
        {!editable ? (
          <EditButton type={"button"} onClick={() => setEditable(true)}>
            <EditGreySvg />
            Edit
          </EditButton>
        ) : (
        ...
        {editable ? (
          <PictureUploadContainer>
            ...
            <ClickToUpload>
              ...
              <ErrorText>{errors["profile_image"]?.message}</ErrorText>
              {/* SVG should be changed when there is an image available */}
              <User />
            </ClickToUpload>
          </PictureUploadContainer>
        ...
        <GridLayout>
          {data.map((el, index) => (
            <InputWithLabel
              ...
              name={el.name}
              type={"text"}
              disable={el.name === "email_address" ? true : !editable}
              register={register}
              inputClass={el.name === "email_address" ? "emailstyle" : null}
