...
const BusinessAddress = () => {
  ...
  // const countriesList = allCountriesDetailed?.map((el) => el?.name);

  // const handleCountry = useCallback(
  //   (selected) => {
  //     // setCountry(allCountriesDetailed.find((el)=> el?.isoCode === selected?.isoCode));
  //     setCountry(selected);
  //     const statesList = allStates
  //       ?.filter(el?.countryCode === country?.isoCode)
  //       ?.map((el) => el?.name);
  ...
  //   (selected) => {
  //     // setState(allStates?.find((el) => el?.isoCode === selected?.isoCode));
  //     setState(selected);
  //     const citiesList = allCities
  //       ?.filter(el?.stateCode === state?.isoCode)
  //       ?.map((el) => el?.name);
  ...
  //   (selected) => {
  //     // setCity(allCities?.find((el) => el?.name === selected.name));
  //     setCity(selected);
  //   },
  //   [state, city]
  // );
          ...
          <LaunchFormContainer>
            <Inputs>
              <TagInputWithSearch
                ...
                suggestionLoading={country === "--" ? true : false}
                fetchingText=""
              ...
              <TagInputWithSearch
                ...
                initialValue={state}
                suggestionLoading={state === "--" ? true : false}
                fetchingText="Select a state"
                fetchFailedText="Could not fetch states"
              />
              <TagInputWithSearch
                label="Cities"
                list={cities}
                getValue={selectCity}
                initialValue={city}
                suggestionLoading={city === "--" ? true : false}
                fetchingText="Select a city"
                fetchFailedText="Could not fetch cities"
              />
              <InputWithLabel
