...
export const checkDocumentInfo = (MemberKYCInfo, documentContainer) => {
  let newContainer = [];
  MemberKYCInfo.forEach((document) => {
    documentContainer.forEach((member) => {
      if (document.memberCode === member.code) {
        let checked = { ...document };
        newContainer.push(checked);
      }
    });
  });
