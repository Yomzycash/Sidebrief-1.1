...
const StaffEntityModal = ({
	...
	useEffect(() => {
		if (entityInfo && cardAction === "edit") {
			...
			setValue("entity_name", entityInfo.entityName, {
				shouldValidate: true,
			});
			...
			setValue("shares", entityInfo.entityShares, {
				shouldValidate: true,
			});
			setValue("timeline", entityInfo.entityTimeline, {
				shouldValidate: true,
			});
			setValue("country", entityInfo.entityCountry, {
				shouldValidate: true,
			});
			setValue("currency", entityInfo.entityCurrency, {
				shouldValidate: true,
			});
			...
			setValue(
				"currency",
				`${countryEntity && countryInfo?.countryCurrency}`
			);
			setValue("fee", "");
		...
		>
			...
			<DetailedSection>
				...
				<DropDown
					...
					defaultValue={
						entityInfo?.entityCountry || countryInfo?.countryISO
					}
					fontSize="clamp(12px, 1.2vw, 14px)"
					height="40px"
